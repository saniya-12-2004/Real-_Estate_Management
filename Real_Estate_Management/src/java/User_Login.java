...
public class User_Login extends HttpServlet {
    ...
    PrintWriter out=resp.getWriter();
        String User_username=req.getParameter("uname");
            ...
            try
    {
       ...
       cn=DriverManager.getConnection("jdbc:mysql://localhost:3306/db_realestate", "root", "root");
        ...
        String sql="select * from tbl_user_signup where email='"+User_username+"' and passwd='"+User_password+"'";
        ResultSet rs=st.executeQuery(sql);
        if(rs.next())
        {
            session.setAttribute("uid", rs.getString("uid"));
            resp.sendRedirect("User_ViewCategory.jsp");
        }
        else
        {
